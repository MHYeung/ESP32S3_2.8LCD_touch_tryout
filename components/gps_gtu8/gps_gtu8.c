...
// Track fix validity per NMEA sentence type to avoid "sticky" valid_fix
// (RMC and GGA arrive separately; we combine their latest states)
static bool s_valid_fix_rmc = false;
static bool s_valid_fix_gga = false;
...
static void handle_gga(char *fields[], int n, gps_fix_t *fix)
{
    ...
    if (fields[6] && fields[6][0])
        fix->fix_quality = atoi(fields[6]);
    if (fields[7] && fields[7][0])
        fix->sats = atoi(fields[7]);
    if (fields[8] && fields[8][0])
...
static void parse_line(const char *line_in)
{
    ...
    else if (strcmp(type, "GPGGA") == 0 || strcmp(type, "GNGGA") == 0)
    {
        ...
        // For GGA, fix quality 0 means "no fix", >0 means we have a fix.
        // Make valid_fix reflect the latest GGA state (so it can also clear).
        if (upd.fix_quality >= 0)
        {
            upd.valid_fix = (upd.fix_quality > 0);
        }
    }
    else
    {
    ...
    if (isfinite(upd.speed_mps))
        s_latest.speed_mps = upd.speed_mps;
    if (isfinite(upd.course_deg))
        s_latest.course_deg = upd.course_deg;

    // If has fix meta, overwrite
    if (upd.sats >= 0)
        s_latest.sats = upd.sats;
    if (isfinite(upd.hdop))
        s_latest.hdop = upd.hdop;
    if (upd.fix_quality >= 0)
        s_latest.fix_quality = upd.fix_quality;

    // Valid flags: accumulate
    // valid_fix must NOT be sticky. Track the latest RMC/GGA fix states separately,
    // then combine them here. This allows valid_fix to clear when GPS loses lock.
    if (is_rmc)
    {
        s_valid_fix_rmc = upd.valid_fix;
    }
    if (is_gga && upd.fix_quality >= 0)
    {
        s_valid_fix_gga = (upd.fix_quality > 0);
    }
    s_latest.valid_fix = (s_valid_fix_rmc || s_valid_fix_gga);
...
esp_err_t gps_gtu8_init(const gps_gtu8_config_t *cfg)
{
    ...
    s_valid_fix_rmc = false;
    s_valid_fix_gga = false;
    xSemaphoreGive(s_lock);
