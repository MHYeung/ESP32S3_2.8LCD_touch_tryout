...
static void menu_bottom_swipe_event_cb(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    lv_indev_t *indev = (lv_indev_t *)lv_event_get_param(e);
    if (!indev || s_transitioning || s_current_page != UI_PAGE_MENU)
        return;

    if (code == LV_EVENT_PRESSED)
    {
        s_menu_swipe_sum.x = 0;
        s_menu_swipe_sum.y = 0;
        s_menu_swipe_armed = true;
    }
    else if (code == LV_EVENT_RELEASED)
    {
        s_menu_swipe_armed = false;
    }
    else if (code == LV_EVENT_PRESSING && s_menu_swipe_armed)
    {
        lv_point_t v;
        lv_indev_get_vect(indev, &v);
        s_menu_swipe_sum.x += v.x;
        s_menu_swipe_sum.y += v.y;

        // Up swipe detected
        if (s_menu_swipe_sum.y < -30 && LV_ABS(s_menu_swipe_sum.y) > (LV_ABS(s_menu_swipe_sum.x) + 10))
        {
            s_menu_swipe_armed = false;
            lv_indev_stop_processing(indev);
            lv_indev_wait_release(indev);
...
static void ui_pages_relayout(void)
{
    ...
    if (s_page_menu)
    {
        ...
        {
            lv_obj_set_pos(s_page_menu, 0, -h);
            lv_obj_add_flag(s_page_menu, LV_OBJ_FLAG_HIDDEN);
        }
    }
    ...
    if (s_menu_bottom_gesture)
    {
        lv_obj_set_size(s_menu_bottom_gesture, lv_pct(100), lv_pct(15));
        lv_obj_align(s_menu_bottom_gesture, LV_ALIGN_BOTTOM_MID, 0, 0);
        if (s_current_page == UI_PAGE_MENU && !s_transitioning)
        {
            lv_obj_clear_flag(s_menu_bottom_gesture, LV_OBJ_FLAG_HIDDEN);
            lv_obj_move_foreground(s_menu_bottom_gesture);
        }
        else
        {
...
{
    if (target == s_current_page || s_transitioning)
        return;
    if (!s_scr)
        return;
    ...
    // DATA -> MENU (open menu)
    if (target == UI_PAGE_MENU && s_current_page == UI_PAGE_DATA)
    {
        anim_obj = s_page_menu;
        lv_obj_clear_flag(s_page_menu, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_y(s_page_menu, -h);
        lv_obj_move_foreground(s_page_menu);
        from_y = -h;
        to_y = 0;
        next_page = UI_PAGE_MENU;
    }
    // MENU -> DATA (close menu)
    else if (target == UI_PAGE_DATA && s_current_page == UI_PAGE_MENU)
    {
        anim_obj = s_page_menu;
        from_y = 0;
        to_y = -h;
    ...
    else if (target == UI_PAGE_SETTINGS && s_current_page == UI_PAGE_MENU)
    {
        ...
        lv_obj_clear_flag(s_page_settings, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_y(s_page_settings, -h);
...
static void create_pages_ui(void)
{
    ...
    // 2. Menu Page (NEW)
    s_page_menu = lv_obj_create(s_scr);
    lv_obj_set_size(s_page_menu, lv_pct(100), lv_pct(100));
    lv_obj_clear_flag(s_page_menu, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_style_pad_all(s_page_menu, 0, 0);
    lv_obj_set_style_border_width(s_page_menu, 0, 0);
    ui_theme_apply_screen(s_page_menu);
    menu_page_create(s_page_menu);
    ...
    s_menu_bottom_gesture = lv_obj_create(s_page_menu);
